...
const SHOPPING_ACTIONS = ['推荐', '看看', '帮我找', '帮我选', '帮我挑', '搜索', '查找', '购买', '买一个', '买一款', '找一款', '选一款', '挑一款', '看看有没有'];
...
const EXPLICIT_RECOMMENDATION_PATTERN = /让我推荐|你推荐|推荐(?:几个|几款|一下)|(?:帮我|给我|随便给我|直接给我)推荐|帮我挑|帮我选|直接推荐|有什么值得买|那你帮我看看/u;
...
export function classifyMessageFallback(message, context = {}) {
  ...
  if (context.pendingProduct && (findIncluded(text, CONFIRMATIONS) || COMMERCE_FOLLOW_UP_PATTERN.test(text) || hasExplicitRecommendationRequest(text))) {
    return {
      scene: 'shopping',
      allowSearch: true,
